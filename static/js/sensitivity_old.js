...
function updateSensitivity() {
    ...
    try {
        ...
        if (typeof getBusinessParams !== 'function' || typeof getFinancialParams !== 'function') {
            console.warn('⚠️ Funciones de parámetros no disponibles, usando datos de prueba');
            displayTestData();
            return;
        }
...
function simulateScenarioMetrics(params, financial) {
    try {
        ...
        if (typeof marketDistribution === 'undefined') {
            console.error('❌ marketDistribution no está definido');
        ...
        for (let year = 2026; year <= 2030; year++) {
            ...
            const yearlyTraffic = (params.initialTraffic || 9100) * Math.pow(1 + (params.trafficGrowth || 60) / 100, yearIndex + 1);
            
            // Calcular conversión creciente
            const conversionRate = Math.min(
                (params.initialConversion || 2) / 100 * Math.pow(1 + (params.conversionGrowthRate || 30) / 100, yearIndex), 
                0.035 // Máximo 3.5%
            );
            
            // Calcular ticket con crecimiento premium
            const ticketSize = (params.avgTicket || 50) * (1 + yearIndex * 0.08);
...
// Función de debug para sensibilidad
function debugSensitivity() {
    console.log('🔍 DEBUG: Estado del análisis de sensibilidad');
    
    // Verificar dependencias
    const dependencies = {
        'marketDistribution': typeof marketDistribution !== 'undefined',
        'getBusinessParams': typeof getBusinessParams === 'function',
        'getFinancialParams': typeof getFinancialParams === 'function',
        'modelData': typeof modelData !== 'undefined'
    };
    ...
    const elements = {
        'sensibilidadBody': document.getElementById('sensibilidadBody') !== null,
        'sensitivityVAN': document.getElementById('sensitivityVAN') !== null,
        'sensitivityTIR': document.getElementById('sensitivityTIR') !== null,
        'sensitivityRevenue': document.getElementById('sensitivityRevenue') !== null,
        'sensitivityRisk': document.getElementById('sensitivityRisk') !== null
    };
    
    console.log('🎯 Elementos HTML:', elements);
    
    // Verificar parámetros si están disponibles
    if (dependencies.getBusinessParams && dependencies.getFinancialParams) {
        try {
            const businessParams = getBusinessParams();
            const financialParams = getFinancialParams();
            console.log('📊 Parámetros business:', businessParams);
            console.log('💰 Parámetros financial:', financialParams);
        } catch (error) {
            console.error('❌ Error obteniendo parámetros:', error);
        }
    }
    
    // Verificar si hay datos de sensibilidad
    if (modelData && modelData.sensitivity) {
        console.log('📈 Datos de sensibilidad:', modelData.sensitivity);
    } else {
        console.log('⚠️ No hay datos de sensibilidad en modelData');
    }
    
    return {
        dependencies,
        elements,
        hasData: !!(modelData && modelData.sensitivity)
    };
}
...
if (typeof window !== 'undefined') {
    ...
    // Ejecutar análisis inicial después de que DOM esté listo
    document.addEventListener('DOMContentLoaded', function() {
        console.log('🎯 Iniciando análisis de sensibilidad en DOM ready...');
        setTimeout(() => {
            if (typeof getBusinessParams === 'function' && typeof getFinancialParams === 'function') {
                updateSensitivity();
            } else {
                console.warn('⚠️ Funciones de parámetros no disponibles aún para sensibilidad');
                // Ejecutar debug para diagnóstico
                debugSensitivity();
            }
        }, 1000); // Dar tiempo a que otras funciones se carguen
    });
}

// Función auxiliar para métricas por defecto
...
function displayTestData() {
    console.log('🧪 Mostrando datos de prueba para sensibilidad');
    ...
    const testScenarios = {
        ...
        'Base': {
            ...
            metrics: {
                economicNPV: 2500000,
                financialNPV: 1700000,
                economicIRR: 15.8,
                financialIRR: 22.3,
                revenue2030: 12500000
            }
        },
        'Optimista': {
            name: 'Optimista',
            ...
            metrics: {
                economicNPV: 5200000,
                financialNPV: 3800000,
                economicIRR: 28.5,
                financialIRR: 38.7,
                revenue2030: 18700000
            }
        },
        'Stress Test': {
            name: 'Stress Test',
    ...
    updateSensitivityDisplay(testScenarios);
    updateSensitivityBanners(testScenarios);
    
    console.log('✅ Datos de prueba mostrados');
}
